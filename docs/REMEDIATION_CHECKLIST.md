...
| DB-004 | Alto | Ciclo operacional | IMPLEMENTADO | `src/Domain/OperationalCycle.php`, `OperationalContext.php`, `tests/domain/` e endpoints integrados | Todas as rotas usam uma única regra `[01:30, 01:30) configurável` e timezone definido | Testes unitários e política estática passaram | Validar timezone no MySQL real |
